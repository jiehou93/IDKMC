    ...
    real*8 radius1,radius2,ran1,mig_step
    real*8 x(3),x1(3),x2(3),vector1(3),vector2(3),parameters1(8),parameters2(8)
    ...
    n1=n-1
    n2=1
    emit=nint(clu(i)%para(7))
    
    formula2=0
    if(iso_eff==1 .AND. (abs(emit)==2 .OR. abs(emit)==3))then
		!考虑同位素效应,跟据2、3组分数量随机选择发射组分
        call RANDOM_NUMBER(ran1)
        if(ran1<clu(i)%formula(2)*1.0/(clu(i)%formula(2)+clu(i)%formula(3)))then
            formula2(2)=emit/abs(emit)
        else
            formula2(3)=emit/abs(emit)
        endif
    else
        !不考虑同位素效应，直接确定发射组分
    ...
    call assign_para(formula1,parameters1)
    call assign_para(formula2,parameters2)
    radius1=parameters1(4)                                   !解离后的半径
    radius2=parameters2(4)

    x1=x                                                    !解离后大团簇的坐标

    mig_step=max(parameters2(8),0.1)                        !防止mig_step=0，导致发射后立刻聚合，陷入死循环
    call RANDOM_NUMBER(ran1)
    orien2=ran1*ndirection+1
